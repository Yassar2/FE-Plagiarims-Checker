document.getElementById("uploadBtn").addEventListener("click", async () => {
    ...
    if (file.size === 0) {
        alert("File kosong tidak bisa diproses!");
        return;
    }
    ...
    try {
        const response = await fetch(
            "https://be-plagiarism-checker-production.up.railway.app/predict",
            {
                method: "POST",
                body: formData
            }
        ...
        if (!contentType.includes("application/json")) {
            ...
            resultDiv.innerHTML = `<p class="text-danger">Server tidak mengembalikan JSON valid</p>`;
            return;
        }
        ...
        if (Array.isArray(data) && data.length > 0) {
            data.forEach((item, index) => {
                ...
                <div class="result-item mb-4 p-4 border rounded-lg">
                    ...
                    <p><strong>Abstract:</strong> ${item.abstract || "Tidak tersedia"}</p>
